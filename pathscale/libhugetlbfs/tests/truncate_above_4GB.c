...
int main(int argc, char *argv[])
{
	...
	verbose_printf("Mapping 3 hpages at offset 0x%" SCNx64 "...", truncate_point);
	...
	*pi = 0;
	...
	verbose_printf("Replacing map at %p with map from offset 0x%" SCNx64 "...",
		       q + 2*hpage_size, buggy_offset);
	p = mmap64(q + 2*hpage_size, hpage_size, PROT_READ|PROT_WRITE,
		 MAP_FIXED|MAP_PRIVATE, fd, buggy_offset);
	if (p != q + 2*hpage_size)
		FAIL("mmap() buggy offset 0x%" SCNx64 "", buggy_offset);
	verbose_printf("done\n");
	pi = p;
	/* Touch the low page with something non-zero */
	*pi = 1;

	verbose_printf("Truncating at 0x%" SCNx64 "...", truncate_point);
