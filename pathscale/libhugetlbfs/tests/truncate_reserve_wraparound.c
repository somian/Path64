...
static unsigned long long read_reserved(void)
{
	...
	ret = fscanf(f, "HugePages_Rsvd: %" SCNd64 "", &count);
	if (ret != 1)
...
int main(int argc, char *argv[])
{
	...
	verbose_printf("Reserve count before map: %" SCNd64 "\n", initial_rsvd);
	...
	verbose_printf("Reserve count after map: %" SCNd64 "\n", read_reserved());

	*q = 0;
	verbose_printf("Reserve count after touch: %" SCNd64 "\n", read_reserved());
	...
	rsvd = read_reserved();
	verbose_printf("Reserve count after truncate: %" SCNd64 "\n", rsvd);
	if (rsvd != initial_rsvd)
		FAIL("Reserved count is not restored after truncate: %" SCNd64 " instead of %" SCNd64 "",
		     rsvd, initial_rsvd);
	...
	rsvd = read_reserved();
	verbose_printf("Reserve count after SIGBUS fault: %" SCNd64 "\n", rsvd);
	if (rsvd != initial_rsvd)
		FAIL("Reserved count is altered by SIGBUS fault: %" SCNd64 " instead of %" SCNd64 "",
		     rsvd, initial_rsvd);

	munmap(p, hpage_size);

	verbose_printf("Reserve count after munmap(): %" SCNd64 "\n",
		       read_reserved());

	close(fd);

	verbose_printf("Reserve count after close(): %" SCNd64 "\n",
		       read_reserved());
