...
#ifdef KEY /* Bug 5091 */
/*
 * Implement F2003 "move_alloc" intrinsic, deallocating "to" if need be,
 * moving allocation from "from" to "to", and setting "from" to "unallocated".
 * Assume that front end calls this only if "from" and "to" are allocatable
 * objects which are compatible in rank and type.
 * from		source allocatable object
 * to		destination allocatable object
 */
void
_MOVE_ALLOC(DopeVectorType *from, DopeVectorType *to) {
  if (to->assoc) {
    AllocHeadType list;
    list.version = ALLOC_VERSION;
    list.imalloc = 0; /* Imitating _DEALLOCATE */
    list.icount = 1;
    list.dv[0] = to;
    _DEALLOC(&list);
  }
  if (from->assoc) {
    DopeAllocType *alloc_info = DOPE_ALLOC_INFO(from);
    memcpy(to, from, DOPE_VECTOR_SIZE(from));
    from->assoc = 0;
    if (from->type_lens.type == DVTYPE_ASCII) {
      from->base_addr.charptr = _cptofcd(0, 0);
    }
    else {
      from->base_addr.a.ptr = 0;
    }
    from->orig_base = 0;
    from->orig_size = 0;
  }
