	...
	size_t		asize)		/* arena size (bytes per process) */
{
	...
	if (arp == NULL) {
                fprintf(stderr,"%s:\n\
    _glio_arena_create(a) could not allocate a memory object of size %" SCNd64 " bytes\n",
	...
	switch (gg->grtype) {
		...
		if ( _shmem_group_inquire != NULL) {
			...
			if (world_plist == NULL) {
                		fprintf(stderr,"%s:\n\
    _glio_arena_create(b) could not allocate a memory object of size %" SCNd64 " bytes\n",
	...
	switch (gg->grtype) {
		...
		if (myrank != 0) {
			...
			if (fname == NULL) {
                		fprintf(stderr,"%s:\n\
    _glio_arena_create(c) could not allocate a memory object of size %" SCNd64 " bytes\n",
		...
		if (myrank == 0) {
			...
			if (fname == NULL) {
                		fprintf(stderr,"%s:\n\
    _glio_arena_create(d) could not allocate a memory object of size %" SCNd64 " bytes\n",
				GLOBERRMSG, (long long)(namelen + 1));
				abort();
			}
...
_glio_malloc(struct glob_f *glob_info, size_t size)
{
	...
	if (ptr == NULL) {
                fprintf(stderr,"%s:\n\
    _glio_malloc() could not allocate a memory object of size %" SCNd64 " bytes\n",
