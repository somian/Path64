	...
	cmp_lyr *cinfo;
{
	...
	if (cinfo->DebugLvl > DEBUG_LVL1) {
		...
		TRC_IT("t_DataSegAddr: 0x%" SCNx64 "", trailer.t_DataSegDAddr);
		TRC_IT("t_NumDataSegD: %" SCNd64 "",   trailer.t_NumDataSegD);
	...
	int	cmpDSegSize;
{
	...
	TRC_LOG("dSegDSize = %d, NumDSegD %d", dSegDSize, 
		 cinfo->NumDSegD);
	TRC_LOG("TmpCache FileAddr 0x%" SCNx64 ", Length %d, CmpLength %d",
		 tmpCache->FileAddr, tmpCache->Length, 
		 tmpCache->CmpLength);
	TRC_LOG("TmpCache SegID %0.8s Comp %d, NextDataSegD 0x%" SCNx64 "",
