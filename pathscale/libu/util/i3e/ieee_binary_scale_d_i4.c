...
#pragma ident "@(#) libu/util/i3e/ieee_binary_scale_d_i4.c	92.1	07/07/99 14:37:09"
...
extern long double _IEEE_BINARY_SCALE_D_I4(long double x, short n);
...
long double
_IEEE_BINARY_SCALE_D_I4(long double x, short n)
{  
	...
	if (fp_class==FP_NORMAL) {
		...
		if (n <= 0) {
			...
			if (mantissa_bits_to_fill >= (int) n) {
				unsigned long	mantissa_bits;
