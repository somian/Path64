...
static const char USMID[] = "@(#) libu/util/i3e/ieee_next_after_d_r.c	92.0	10/08/98 14:57:41";
...
extern long double _IEEE_NEXT_AFTER_D_R(long double x, double y);
 ...
 *
 */
#pragma _CRI duplicate _IEEE_NEXT_AFTER_D_R as _IEEE_NEXT_AFTER_16_8
long double
_IEEE_NEXT_AFTER_D_R(long double x, double y)
{
	...
	if (xfpclas == FP_NAN) {
		...
		return x;
	} else if (x==(long double) y) {
		return x;
	} else if (xfpclas == FP_ZERO) {
		union _ieee_ldouble x_val;
		x_val.dword = LDBL_MIN;
		x_val.parts.sign	 = x > (long double) y;
		 ...
		 */
		x_val.lword[1] += (x > savey) ? add1 : add2;

		if ((f.parts.mantissa_low == add3) ||
		   (x_val.parts.mantissa_low == add3)) {
			x_val.lword[0] += (x>savey) ? add1 : add2;
		}
