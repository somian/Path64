...
#pragma ident "@(#) libu/ieg/mpp/cri2ieg_77.c	92.1	06/25/99 14:59:36"
...
struct _dsz_s   G@IEG@SZ = {    /* IEEE/generic */
        /* number of types */
                9,
/*      none  int   real  dble  cmplx log   char  sint  64-64	*/
        64,   32,   32,   64,   64,   32,   8,    16,	64 };
 
struct _dal_s   G@IEG@AL = {
			0,			/* no alignment */
			0,			/* UNUSED */
			0,			/* no alignment boundary */
			0,			/* no alignment character */
};
 

extern _f_int
CRI2IEG(_f_int *type, _f_int *num, void *forn, _f_int *bitoff, void *native,
	_f_int *stride, _f_int *intlen, _f_int *extlen, _fcd nativech);

/*
 *	The CRI2IEG_77 subroutine converts Fortran data types on a system
 *	using IEEE (T3D) floating-point representation to a system
 *	using generic 32 bit IEEE floating point representation.
 *
 *	Call from Fortran:
 *
 *	INTEGER CRI2IEG_77
 *
 *	IERR = CRI2IEG_77(TYPE, NUM, FORN, BITOFF, NATIVE, STRIDE, [, NATIVECH])
 *
 ...
 *				or length to receive the converted 32 bit IEEE 
 *				data.
 *
 *		This variable contains the target Cray MPP systems array or 
 *		32 bit IEEE systems array to received the converted data, 
 *		whichever is not native to the running system.
 *
 *	BITOFF
 *
 ...
 *	NATIVE
 *
 *		T3D systems:  	Variable or array containing the values to be
 *				converted if type is not CHARACTER.
 *
 *		This variable contains the target Cray MPP systems array or 
 ...
 *
 *		T3D systems:  	Variable or array containing the values to be
 *				converted if type is CHARACTER.
 *
...
)
{
	...
	return( CRI2IEG(&type90, num, forn, bitoff, native, stride, &intlen,
		&extlen, nativech) );
}
