...
/* USMID @(#) libfi/mpp_reduce/minval_p.h	92.0	10/08/98 14:37:14 */
...
Pragma(duplicate XGLUE(_MINVAL_JP,RANK) as XGLUE(_MINVAL_SP,RANK))

void
XGLUE(_MINVAL_JP,RANK)(DopeVectorType * result,
	    ...
	    DopeVectorType * mask)
{
	...
	if (source->type_lens.type == DVTYPE_REAL) {
	    XGLUE(XGLUE(MINVAL_I_SP,RANK),@)(result_sdd_ptr, source_sdd_ptr,
			dim, mask_sdd_ptr, &mask_flag, res_extents, 
			src_extents, blkcnts, dim_bcnt);
	} else {
	    XGLUE(XGLUE(MINVAL_I_JP,RANK),@)(result_sdd_ptr, source_sdd_ptr,
			dim, mask_sdd_ptr, &mask_flag, res_extents,
			src_extents, blkcnts, dim_bcnt);
	}
