...
/* USMID @(#) libfi/mpp_reduce/minloc_s.h	92.0	10/08/98 14:37:14 */
...
Pragma(duplicate XGLUE(_MINLOC_JS,RANK) as XGLUE(_MINLOC_SS,RANK))

void
XGLUE(_MINLOC_JS,RANK)(DopeVectorType * result,
	   ...
	   DopeVectorType * mask)
{
	...
	if (!_is_shared_pointer(source->base_addr.a.ptr)) {
		...
		MINLOC_S@(result, source, mask);
	    } else {
		MINLOC_J@(result, source, mask);
	    }
	} else {
	    ...
	    if (mask_flag != 0) {
		if (source->type_lens.type == DVTYPE_REAL) {
		    XGLUE(XGLUE(MINLOC_SP,RANK),@)(result_base_ptr,
			source_sdd_ptr, mask_sdd_ptr, &mask_flag, extents);
		} else {
		    XGLUE(XGLUE(MINLOC_JP,RANK),@)(result_base_ptr,
			source_sdd_ptr, mask_sdd_ptr, &mask_flag, extents);
		}
	    } else {
		if (source->type_lens.type == DVTYPE_REAL) {
		    XGLUE(XGLUE(MINLOC_NM_SP,RANK),@)(result_base_ptr,
			source_sdd_ptr, extents);
		} else {
		    XGLUE(XGLUE(MINLOC_NM_JP,RANK),@)(result_base_ptr,
			source_sdd_ptr, extents);
		}
	    }
	}
