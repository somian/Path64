...
/* USMID @(#) libfi/mpp_reduce/minval_s.h	92.0	10/08/98 14:37:14 */
...
Pragma (duplicate XGLUE(_MINVAL_JS,RANK) as XGLUE(_MINVAL_SS,RANK))

void
XGLUE(_MINVAL_JS,RANK)(DopeVectorType * result,
	    ...
	    DopeVectorType * mask)
{
	...
	if (!_is_shared_pointer(source->base_addr.a.ptr)) {
		...
		_MINVAL_S(result, source, dim, mask);
	    } else {
		_MINVAL_J(result, source, dim, mask);
	    }
	} else {
	    ...
	    if (source->type_lens.type == DVTYPE_REAL) {
		XGLUE(XGLUE(MINVAL_I_SS,RANK),@)(result_base_ptr,
			source_sdd_ptr, dim, mask_sdd_ptr, &mask_flag,
			res_extents, src_extents, blkcnts, dim_bcnt);
	    } else {
		XGLUE(XGLUE(MINVAL_I_JS,RANK),@)(result_base_ptr,
			source_sdd_ptr, dim, mask_sdd_ptr, &mask_flag,
			res_extents, src_extents, blkcnts, dim_bcnt);
	    }
	}
