...
#pragma ident "@(#) libfi/prodsum/presum@.c	92.2	07/13/99 10:46:27"
...
*  routines SUM_x@, PROD_x@, SUM_x, and PROD_x.  This function is a      *
*  Fortran-callable function with call-by-address calling sequence).     *
*                                                                        *
...
*                                                                        *
*  pdim            (address of) integer (optional) of number of the      *
...
#define NAME _PRESUM		/* name of routine */
#ifdef	_UNICOS
#pragma _CRI inline _PRESUM
#pragma _CRI duplicate _PRESUM as PRESUM@
#endif
void
NAME(int *flag, DopeVectorType * RESULT, DopeVectorType * MATRIX_A,
	int *DIM, DopeVectorType * MASK, int *ndim, int *dimarg,
	int *dima, int *maskarg, int *scalar, long *loca,
...
#pragma CRI     shortloop
	...
	if (DIM != NULL) {	/* was DIM argument given? */
		*dimarg = _btol(1);	/* dimarg = .TRUE. */
		dim = *DIM;		/* dimension to sum over   */
		*dima = *DIM;		/* dimension to sum over   */
	} else {
		*dimarg = _btol(0);	/* dimarg = .FALSE. */
	}
	if (MASK != NULL) {
		*maskarg = _btol(1);	/* maskarg = .TRUE. */
		locmc = (void *) MASK->base_addr.a.ptr;
		*locm = (long) locmc;
		*sizem = (long) MASK->type_lens.int_len >> 3;
		*loc4m = (long) locmc;
...
#pragma CRI     shortloop
		...
		for (i = 0; i < *ndim; i++) {	/* per dimension of A */
			...
			if( MASK->n_dim == MATRIX_A->n_dim ) {
				/* number of elements in mask */
				limm[i] = MASK->dimension[i].extent;
				/* increment in mask */
				incm[i] = MASK->dimension[i].stride_mult;
			} else {
				limm[i] = 1;
				incm[i] = 0;
			}
			if (MASK->type_lens.int_len > BITS_PER_WORD)
				incm[i] >>= 1;
