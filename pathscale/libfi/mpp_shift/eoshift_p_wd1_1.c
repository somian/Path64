...
#pragma ident "@(#) libfi/mpp_shift/eoshift_p_wd1_1.c	92.1	07/13/99 10:44:06"
 ...
 *		CALL _EOSHIFT_JP1(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *		CALL _EOSHIFT_LP1(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *		CALL _EOSHIFT_SP1(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *
...
#pragma duplicate _EOSHIFT_JP1 as _EOSHIFT_LP1
#pragma duplicate _EOSHIFT_JP1 as _EOSHIFT_SP1
 ...
 */
	...
	if (special) {
	    EOSHIFT_SPEC_WD1_P1@ ( result_sdd_ptr, source_sdd_ptr,
		&dim_val, src_extents, &shftval, &bndval);
	} else {
	    EOSHIFT_WD1_P1@ ( result_sdd_ptr, source_sdd_ptr, shft_array,
		bnd_array, &dim_val, src_extents, shft_extents,
