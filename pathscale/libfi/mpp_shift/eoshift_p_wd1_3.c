...
#pragma ident "@(#) libfi/mpp_shift/eoshift_p_wd1_3.c	92.1	07/13/99 10:44:06"
 ...
 *		CALL _EOSHIFT_JP3(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *		CALL _EOSHIFT_LP3(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *		CALL _EOSHIFT_SP3(RESULT, SOURCE, SHIFT, BOUNDARY, DIM)
 *
...
#pragma duplicate _EOSHIFT_JP3 as _EOSHIFT_LP3
#pragma duplicate _EOSHIFT_JP3 as _EOSHIFT_SP3
 ...
 */
	...
	if (special) {
	    EOSHIFT_SPEC_WD1_P3@ ( result_sdd_ptr, source_sdd_ptr,
		&dim_val, src_extents, &shftval, &bndval);
	} else {
	    EOSHIFT_WD1_P3@ ( result_sdd_ptr, source_sdd_ptr, shift_sdd_ptr,
