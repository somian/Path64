...
#pragma ident "@(#) libfi/mpp_scan/scansum_ss_6.c	92.1	07/13/99 10:42:37"
...
/*
 *	Compiler generated call: CALL _SCANSUM_SS6(RES, SRC, STOP, DIM, MASK)
 *
...
_SCANSUM_SS6 (	DopeVectorType	*result,
...
#include "segmented_scan_s.h"
	...
	if (stop_flag == 1) {
	    if (mask_flag == 1) {
		SCANSUM_MASK_SS6@ (result_base_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents);
	    } else {
		SCANSUM_NOMASK_SS6@ (result_base_ptr, source_sdd_ptr,
