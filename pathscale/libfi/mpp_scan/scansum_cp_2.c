...
#pragma ident "@(#) libfi/mpp_scan/scansum_cp_2.c	92.1	07/13/99 10:42:37"
...
/*
 *	Compiler generated call: CALL _SCANSUM_CP2(RES, SRC, STOP, DIM, MASK)
 *
...
void
_SCANSUM_CP2 (	DopeVectorType	*result,
...
#include "segmented_scan_p.h"
	...
	if (stop_flag > 0) {
	    if (mask_flag == 1) {
		SCANSUM_MASK_CP2@ (result_sdd_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents,
			blkcnts);
	    } else {
		SCANSUM_NOMASK_CP2@ (result_sdd_ptr, source_sdd_ptr,
