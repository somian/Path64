...
#pragma ident "@(#) libfi/mpp_scan/scanmax_ss_1.c	92.1	07/13/99 10:21:33"
...
/*
 *	Compiler generated call: CALL _SCANMAX_SS1(RES, SRC, STOP, DIM, MASK)
 *
...
_SCANMAX_SS1 (	DopeVectorType	*result,
...
#include "segmented_scan_s.h"
	...
	if (stop_flag == 1) {
	    if (mask_flag == 1) {
		SCANMAX_MASK_SS1@ (result_base_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents);
	    } else {
		SCANMAX_NOMASK_SS1@ (result_base_ptr, source_sdd_ptr,
