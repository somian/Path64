...
#pragma ident "@(#) libfi/mpp_scan/scanmin_sp_1.c	92.1	07/13/99 10:21:33"
...
/*
 *	Compiler generated call: CALL _SCANMIN_SP1(RES, SRC, STOP, DIM, MASK)
 *
...
void
_SCANMIN_SP1 (	DopeVectorType	*result,
...
#include "segmented_scan_p.h"
	...
	if (stop_flag > 0) {
	    if (mask_flag == 1) {
		SCANMIN_MASK_SP1@ (result_sdd_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents,
			blkcnts);
	    } else {
		SCANMIN_NOMASK_SP1@ (result_sdd_ptr, source_sdd_ptr,
