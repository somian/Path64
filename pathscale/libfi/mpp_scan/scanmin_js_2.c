...
#pragma ident "@(#) libfi/mpp_scan/scanmin_js_2.c	92.1	07/13/99 10:21:33"
...
/*
 *	Compiler generated call: CALL _SCANMIN_JS2(RES, SRC, STOP, DIM, MASK)
 *
...
_SCANMIN_JS2 (	DopeVectorType	*result,
...
#include "segmented_scan_s.h"
	...
	if (stop_flag == 1) {
	    if (mask_flag == 1) {
		SCANMIN_MASK_JS2@ (result_base_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents);
	    } else {
		SCANMIN_NOMASK_JS2@ (result_base_ptr, source_sdd_ptr,
