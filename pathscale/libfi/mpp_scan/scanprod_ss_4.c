...
#pragma ident "@(#) libfi/mpp_scan/scanprod_ss_4.c	92.1	07/13/99 10:42:37"
...
/*
 *	Compiler generated call: CALL _SCANPROD_SS4(RES, SRC, STOP, DIM, MASK)
 *
...
_SCANPROD_SS4 (	DopeVectorType	*result,
...
#include "segmented_scan_s.h"
	...
	if (stop_flag == 1) {
	    if (mask_flag == 1) {
		SCANPROD_MASK_SS4@ (result_base_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents);
	    } else {
		SCANPROD_NOMASK_SS4@ (result_base_ptr, source_sdd_ptr,
