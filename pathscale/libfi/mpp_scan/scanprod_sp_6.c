...
#pragma ident "@(#) libfi/mpp_scan/scanprod_sp_6.c	92.1	07/13/99 10:42:37"
...
/*
 *	Compiler generated call: CALL _SCANPROD_SP6(RES, SRC, STOP, DIM, MASK)
 *
...
void
_SCANPROD_SP6 (	DopeVectorType	*result,
...
#include "segmented_scan_p.h"
	...
	if (stop_flag > 0) {
	    if (mask_flag == 1) {
		SCANPROD_MASK_SP6@ (result_sdd_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents,
			blkcnts);
	    } else {
		SCANPROD_NOMASK_SP6@ (result_sdd_ptr, source_sdd_ptr,
