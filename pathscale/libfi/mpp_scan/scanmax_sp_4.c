...
#pragma ident "@(#) libfi/mpp_scan/scanmax_sp_4.c	92.1	07/13/99 10:21:33"
...
/*
 *	Compiler generated call: CALL _SCANMAX_SP4(RES, SRC, STOP, DIM, MASK)
 *
...
void
_SCANMAX_SP4 (	DopeVectorType	*result,
...
#include "segmented_scan_p.h"
	...
	if (stop_flag > 0) {
	    if (mask_flag == 1) {
		SCANMAX_MASK_SP4@ (result_sdd_ptr, source_sdd_ptr,
			stop_sdd_ptr, &dim_val, mask_sdd_ptr, src_extents,
			blkcnts);
	    } else {
		SCANMAX_NOMASK_SP4@ (result_sdd_ptr, source_sdd_ptr,
