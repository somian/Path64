...
#pragma ident "@(#) libfi/mpp_pre/preprod_ss_1.c	92.1	07/09/99 15:38:17"
...
/*
 *	Compiler generated call: CALL _PREPROD_SS1(RESULT, SOURCE, DIM, MASK)
 *
...
_PREPROD_SS1 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_SS1@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREPROD_NOMASK_SS1@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
