...
#pragma ident "@(#) libfi/mpp_pre/preprod_cs_6.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREPROD_CS6(RESULT, SOURCE, DIM, MASK)
 *
...
_PREPROD_CS6 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_CS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREPROD_NOMASK_CS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
