...
#pragma ident "@(#) libfi/mpp_pre/premax_js_6.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREMAX_JS6(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *
 *	Description:
 *		This is the MPP single PE version of PREMAX. This
...
_PREMAX_JS6 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREMAX_MASK_JS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREMAX_NOMASK_JS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
