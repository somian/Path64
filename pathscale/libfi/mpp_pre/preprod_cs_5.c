...
#pragma ident "@(#) libfi/mpp_pre/preprod_cs_5.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREPROD_CS5(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *	         of rank 5 with a data type of 128-bit complex.
 *
 *	Arguments:
...
_PREPROD_CS5 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_CS5@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREPROD_NOMASK_CS5@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
