...
#pragma ident "@(#) libfi/mpp_pre/preprod_cs_2.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREPROD_CS2(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *	         of rank 2 with a data type of 128-bit complex.
 *
 *	Arguments:
 ...
 *
 *	Description:
 *		This is the MPP single PE version of PREPROD. This
...
_PREPROD_CS2 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_CS2@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREPROD_NOMASK_CS2@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
