...
#pragma ident "@(#) libfi/mpp_pre/presum_js_2.c	92.1	07/09/99 15:38:17"
...
/*
 *	Compiler generated call: CALL _PRESUM_JS2(RESULT, SOURCE, DIM, MASK)
 *
...
_PRESUM_JS2 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PRESUM_MASK_JS2@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PRESUM_NOMASK_JS2@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
