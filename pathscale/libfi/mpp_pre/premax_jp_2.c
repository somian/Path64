...
#pragma ident "@(#) libfi/mpp_pre/premax_jp_2.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREMAX_JP2(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *	         of rank 2 with a data type of 64-bit integer or
 ...
 *
 *      Description:
 *		This is the MPP version of PREMAX. This particular
...
void
_PREMAX_JP2 (	DopeVectorType	*result,
...
#include "parallel_prefix_p.h"
	...
	if (mask_flag == 1)
	    PREMAX_MASK_JP2@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents, blkcnts);
	else
	    PREMAX_NOMASK_JP2@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		src_extents, blkcnts);
