...
#pragma ident "@(#) libfi/mpp_pre/preprod_ss_6.c	92.1	07/09/99 15:38:17"
...
/*
 *	Compiler generated call: CALL _PREPROD_SS6(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *	         of rank 6 with a data type of 64-bit real.
 *
 *	Arguments:
...
_PREPROD_SS6 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_SS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREPROD_NOMASK_SS6@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
