...
#pragma ident "@(#) libfi/mpp_pre/premin_js_3.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREMIN_JS3(RESULT, SOURCE, DIM, MASK)
 *
...
_PREMIN_JS3 (	DopeVectorType	*result,
...
#include "parallel_prefix_s.h"
	...
	if (mask_flag == 1)
	    PREMIN_MASK_JS3@ (result_base_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents);
	else
	    PREMIN_NOMASK_JS3@ (result_base_ptr, source_sdd_ptr, &dim_val,
		src_extents);
}
