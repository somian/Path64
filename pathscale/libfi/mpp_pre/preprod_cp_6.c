...
#pragma ident "@(#) libfi/mpp_pre/preprod_cp_6.c	92.1	07/09/99 15:35:58"
...
/*
 *	Compiler generated call: CALL _PREPROD_CP6(RESULT, SOURCE, DIM, MASK)
 *
...
void
_PREPROD_CP6 (	DopeVectorType	*result,
...
#include "parallel_prefix_p.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_CP6@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents, blkcnts);
	else
	    PREPROD_NOMASK_CP6@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		src_extents, blkcnts);
