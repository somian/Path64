...
#pragma ident "@(#) libfi/mpp_pre/presum_jp_2.c	92.1	07/09/99 15:38:17"
...
/*
 *	Compiler generated call: CALL _PRESUM_JP2(RESULT, SOURCE, DIM, MASK)
 *
 ...
 *
 *      Description:
 *		This is the MPP version of PRESUM. This particular
...
void
_PRESUM_JP2 (	DopeVectorType	*result,
...
#include "parallel_prefix_p.h"
	...
	if (mask_flag == 1)
	    PRESUM_MASK_JP2@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents, blkcnts);
	else
	    PRESUM_NOMASK_JP2@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		src_extents, blkcnts);
