...
#pragma ident "@(#) libfi/mpp_pre/preprod_jp_6.c	92.1	07/09/99 15:38:17"
...
/*
 *	Compiler generated call: CALL _PREPROD_JP6(RESULT, SOURCE, DIM, MASK)
 *
...
void
_PREPROD_JP6 (	DopeVectorType	*result,
...
#include "parallel_prefix_p.h"
	...
	if (mask_flag == 1)
	    PREPROD_MASK_JP6@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		mask_sdd_ptr, src_extents, blkcnts);
	else
	    PREPROD_NOMASK_JP6@ (result_sdd_ptr, source_sdd_ptr, &dim_val,
		src_extents, blkcnts);
