...
class initial_sys_state:
    ...
    def __init__(self):
        ...
        self.pkg_location = installables.pkgdirs
        self.tarball_location = installables.tarballs
        self.sub_server_location = installables.sub_server_pkgs
        self.arch = get_arch()
        
        # Find previous installations
        self.prev_tar_path_list = find_prev_tarinst ([default_dir])
        self.prev_pkg_path_list = pkg.create_pkg_set(distro.type)
        self.prev_pkg_path_list = self.prev_pkg_path_list.find_prev_inst()
        if len(self.prev_tar_path_list) > 0 or len(self.prev_pkg_path_list) > 0:
        ...
        prev_pkg_prefix = list()
        for pkg_path in self.prev_pkg_path_list :
            prefix = pkg_path.prefix
            if prefix not in prev_pkg_prefix :
               prev_pkg_prefix.append (prefix)
        self.prev_sub_list = find_prev_subs (self.prev_tar_path_list, prev_pkg_prefix)
...
def arch_match (fn, arch):
    """Return true if the package in fn matches the architecture."""

    if (arch == 'i386' or
        arch == 'i586' or
        arch == 'i686'):
        if (fn.endswith ('.i386.' + pkg_suffix) or
            fn.endswith ('.i586.' + pkg_suffix) or
            fn.endswith ('.i686.' + pkg_suffix) or
            fn.endswith ('.noarch.' + pkg_suffix)):
            return True
    elif (arch == 'x86_64'):
        if (fn.endswith ('.i386.' + pkg_suffix) or
            fn.endswith ('.i586.' + pkg_suffix) or
            fn.endswith ('.i686.' + pkg_suffix) or
            fn.endswith ('.x86_64.' + pkg_suffix) or
            fn.endswith ('.noarch.' + pkg_suffix)):
            return True
...
def pkg_owned (path):
    ...
    if (initial_sys_state.pkg_suffix == 'rpm'):
        command_prefix = 'rpm -qf '
    else:
        command_prefix = 'dpkg -S '
    pkg_query = command_prefix + path
    pkg_pipe = os.popen (pkg_query)
    pkg_result = pkg_pipe.readlines()
    pkg_pipe.close()

    if len (pkg_result) == 0 or pkg_result[0].find ('not owned') != -1 or pkg_result[0].startswith ('error:') or pkg_result[0].find ('not found') != -1:
        return False
...
def find_prev_tarinst (search_path):
    """Find the previous installations that were made from the tarball installer."""

    ret_list = list()
    for dir in search_path:
        if os.path.isdir (dir):
            # TODO: If the path exists and is not owned by any package
            if os.path.exists (os.path.join (dir, 'bin/pathcc')) and not pkg_owned (os.path.join (dir, 'bin/pathcc')):
                ret_list.append (dir)
...
def find_prev_subs (tar_path, pkg_path):
    ...
    if pkg_path and len(pkg_path) > 0:
        prev_path_list.extend (pkg_path[:])
...
# modify_ld_conf
#
# Modify /etc/ld.so.conf so that it contains the correct path to the
...
# To do: Put searchpath in /etc/ld.so.conf.d/pathscale-<compiler_ver>.conf
#        Then it would be easy to remove if uninstalling later on: 
#        os.remove ('/etc/ld.so.conf.d/pathscale-' + compiler_ver + '.conf')

def modify_ld_conf (sys_state, inst_sel):
    """Fix up /etc/ld.so.conf for the user."""

    try:
        conf_in = open ('/etc/ld.so.conf')
    except:
        return False
    ...
    try:
        conf_out = open ('/etc/ld.so.conf', 'w')
    except:
        return False
...
def fresh_install_view (scr, sys_state, inst_sel):
    ...
    if install_choice == 1:
        ...
        while len (find_prev_tarinst ([old_tar])) == 0:
...
def uninstall_setup_view (scr, sys_state, inst_sel):
    ...
    if sys_state.root_access :
        for installed_path in sys_state.prev_pkg_path_list:
            uninst_choice_list.append (str(installed_path))
    uninst_choice_list.append ('Another location')

    pkg_index = len (sys_state.prev_tar_path_list)

    if sys_state.root_access :
        fillin_index = pkg_index + len (sys_state.prev_pkg_path_list)
    else :
        fillin_index = pkg_index 
        ...
        if c:
            if i < pkg_index:
                ...
                if i < fillin_index:
                    ...
                    pkgs_to_remove.append (pkgs_matching (seldir, selver))
                else:
    ...
    if not os.environ.has_key('DONT_INSTALL'):
        ...
        for pkg_set in pkgs_to_remove:
            remove_pkg_set (pkg_set)
        
    display_centered (scr, max_y - 2, '                                                                ')
    display_centered (scr, max_y - 2, 'Press any key to continue.')
    scr.getch()
    return ( len(tarballs_to_remove) + len(pkgs_to_remove) ) > 0
...
def show_inst_choices_view (scr, sys_state):
    ...
    tmp_y = 8
    if len (sys_state.pkg_location) > 0:
        scr.addstr (tmp_y, 5, 'RPM Directories:')
        for entry in sys_state.pkg_location:
            tmp_y += 1
...
def ask_mount_iso (scr, sys_state):
    ...
    sys_state.pkg_location.extend (installables.pkgdirs[:])
    sys_state.tarball_location.extend (installables.tarballs[:])
    sys_state.sub_server_location.extend (installables.sub_server_pkgs[:])
...
def get_inst_type_view(scr, sys_state, inst_selection):
    ...
    if inst_choice == -1:
        ...
        # Fill the pkg field of the installation set
        for entry in sys_state.pkg_location:
            for root in [entry, os.path.join(entry,'i386'), os.path.join(entry,'x86_86')] :
                if os.path.isdir(root) :
                    for pkg in os.listdir(root):                
                        if arch_match (pkg, sys_state.arch):
                            inst_selection.add_pkg (pkg_basename (os.path.join (root, pkg)), 
                                os.path.join (root, pkg), True, False, False)
    else:
...
def get_inst_type(scr, sys_state, inst_selection):
    ...
    if ((len (sys_state.pkg_location) > 0 and sys_state.root_access) or
        ...
        if (len (sys_state.pkg_location) > 0 or
...
def determine_components_view (scr, sys_state, inst_selection):
    ...
    for pkg in inst_selection.pkg_list:
        if pkg.basename == 'pathscale-c' and choices[0]:
            pkg.selected = True
        elif pkg.basename == 'pathscale-c++' and choices[1]:
            pkg.selected = True
        elif pkg.basename == 'pathscale-f90' and choices[2]:
            pkg.selected = True
        elif pkg.basename == 'pathscale-pathdb' and choices[4]:
            pkg.selected = True
        elif ((pkg.basename == 'pathscale-base' or
               pkg.basename.startswith ('pathscale-compilers-') or
               pkg.basename == 'pathscale-pathopt2') and
              (choices[0] or choices[1] or choices[2] or choices[3] or choices[4])):
            pkg.selected = True
        elif (pkg.basename == 'pathscale-sub-client' and
...
# reselect_pkgs
#
# Select packages in sel.pkg_list for installation if previous versions of
...
def reselect_pkgs (sel):
    c_installed = pkg_installed ('pathscale-c')
    cpp_installed = pkg_installed ('pathscale-c++')
    f_installed = pkg_installed ('pathscale-f90')
    dbg_installed = pkg_installed ('pathscale-pathdb')

    for pkg in sel.pkg_list:
        if pkg.basename == 'pathscale-c':
            if c_installed:
                pkg.selected = True
        elif pkg.basename == 'pathscale-c++':
            if cpp_installed:
                pkg.selected = True
        elif pkg.basename == 'pathscale-f90':
            if f_installed:
                pkg.selected = True
        elif pkg.basename == 'pathscale-pathdb':
            if dbg_installed:
                pkg.selected = True
        elif pkg.basename != 'pathscale-compilers-libs':
...
def setup_upgrade (sys_state, sel):
    ...
    # If it's a tarball, they get a tarball again. If it's pkgs, they
    # get the same set of pkgs. No checking the subscription; I'm
    # going to trust them here.

    if len (sys_state.prev_pkg_path_list) > 0 and len (sys_state.pkg_location) > 0 and len (sel.pkg_list) > 0:
        # Figure out which ones are installed
        reselect_pkgs (sel)
        sel.inst_prefix = sys_state.prev_pkg_path_list[0].prefix
...
#       len(os.listdir(inst_sel.inst_prefix) == 0 if ok to install_tarball
            ...
            if pkg_total:
                for pkg in inst_sel.pkg_list:
                    if pkg.selected:
                        if inst_sel.upgrade:
                            upgrade_pkg_set ([pkg.pkg_path], inst_sel.inst_prefix)
                        else:
                            install_pkg_set ([pkg.pkg_path], inst_sel.inst_prefix)
                        pkg_count += 1
                        fill_bar (scr, 8, 5, max_x - 10, float (pkg_count) / float (pkg_total))
                        scr.refresh()
        if inst_sel.install_sub_server:
            if (inst_sel.sub_server_path and os.path.exists (inst_sel.sub_server_path)):
                ...
                scr.refresh()
                if inst_sel.upgrade:
                    upgrade_pkg_set ([inst_sel.sub_server_path], inst_sel.inst_prefix)
                else:
                    install_pkg_set ([inst_sel.sub_server_path], inst_sel.inst_prefix)
...
def post_config_view (scr, sys_state, inst_sel):
    ...
    if len (inst_sel.tar_list) > 0:
        ...
        if sys_state.root_access:
            if sys_state.arch == 'x86_64':
                scr.addstr (dy + 6, 10, 'Add the following lines to /etc/ld.so.conf:')
                scr.addstr (dy + 8, 20, inst_sel.inst_prefix + '/lib/' + compiler_ver)
                scr.addstr (dy + 9, 20, inst_sel.inst_prefix + '/lib/32/' + compiler_ver)
            else:
                scr.addstr (dy + 6, 10, 'Add the following line to /etc/ld.so.conf:')
...
# modifying /etc/ld.so.conf, for example.
...
def main (stdscr):
    ...
    try:
        ...
        if not inst_selection.upgrade:
            if len (inst_selection.pkg_list) > 0:
        ...
        if (len (inst_selection.pkg_list) == 0 and
